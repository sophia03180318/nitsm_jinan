...
@Slf4j
@Component("bhmPowerAdapter")
public class BhmPowerAdapter  extends AssetIpAdd implements IAdapter<JSONArray> {
    ...
    public void dispose(JSONArray data) {
        List<CollectBhmPowerEntity> queueList = JSONUtil.toList(data, CollectBhmPowerEntity.class);
        ...
        Future<Integer> future=excutorService.submit(new Callable<Integer>() {
            ...
            public Integer call() throws Exception {
                ...
                for (CollectBhmPowerEntity item : queueList) {
                    thresholdDisposePool.execute(() -> {
                        try {
                            item.setCollectCode(collectCode);
                            setAssetIp(item);
                            item.setCount(queueList.size());
