...
/**
 * @author: hhw
 * @description: XunjianReceiver 主要是用来
 * @date: 2025-05-26  11:40
 * @since: 2.1.6.0
 */
//@Component
public class XunjianReceiver {
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                List<byte[]> thresholdList = connection.bLPop(0, RedisQueueConst.XUNJIAN_QUEUE.getBytes());
                ...
                AppLogUtils.buildLogError(LogFunctionEnum.COLLECT_DATA_PARSER, "巡检处理调度被中断", e);
            } finally {
                //检查连接有效性
                if (!AppRedisUtils.verifyRedisConn(connection)) {
                    AppLogUtils.buildLogInfo(LogFunctionEnum.COLLECT_DATA_PARSER, "巡检处理Redis连接已经失效，重新建立连接", "");
