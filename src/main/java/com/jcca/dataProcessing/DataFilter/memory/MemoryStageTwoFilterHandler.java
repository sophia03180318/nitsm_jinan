 ...
 */
@Component("memoryStageTwoFilterHandler")
public class MemoryStageTwoFilterHandler extends IFilterHandler<CollectMemoryEntity> {
    ...
    public boolean handler(CollectMemoryEntity info) {
        AppLogUtils.buildLogInfo(LogFunctionEnum.DATA_PROCESS_SINGLE, "内存二阶阈值信息过滤处理类", info.getAssetIp());
        ChangeInfo changeInfo = info.getMaps().get(StatusInfoChangeTypeEnum.status_memoryState.getCode());

        String eventRedisKey = StatusInfoChangeTypeEnum.event_memory_sectionTwo.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_memory_sectionTwo.getCode(),StatusInfoChangeTypeEnum.SECTION_TWO.getCode(),null);
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_memory_sectionTwo.getDescr(), changeInfo.getValue(), keyWord, threshold.getTwoLevelValue());
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_memory_sectionTwo.getCode(),StatusInfoChangeTypeEnum.SECTION_TWO_VAL.getCode(),null, status, info, changeInfo);
