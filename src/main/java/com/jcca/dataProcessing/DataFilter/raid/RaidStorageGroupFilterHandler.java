 ...
 * @since 2.1.0.0
 */
@Component("raidStorageGroupFilterHandler")
public class RaidStorageGroupFilterHandler extends IFilterHandler<DiskEntity> {
    ...
    public boolean handler(DiskEntity info) {
        if (info.getDiskType() == 0) {
            String redisKey = info.getAssetIp() + ":" + info.getAssetId() + ":" + StatusInfoChangeTypeEnum.status_raid_group.getCode() + ":" + info.getName();
