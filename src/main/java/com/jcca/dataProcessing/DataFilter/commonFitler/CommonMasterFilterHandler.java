...
public class CommonMasterFilterHandler extends IFilterHandler<ItsmQueueEntity> {
    ...
    public boolean handler(ItsmQueueEntity info) {
        ...
        Boolean flag = eventInfoChangeManagerService.infoIschangeFirst(redisKey, mapKey, info.getOldHostType());
        if (flag == null || flag) {
            ChangeInfo changeInfo = new ChangeInfo();
            changeInfo.setValue(info.getOldHostType());
            changeInfo.setRedisKey(redisKey);
            changeInfo.setMapKey(mapKey);
            changeInfo.setCollectTime(new Date());
            info.getMaps().put(mapKey, changeInfo);

            if(StrUtil.isEmpty(info.getOldHostType())){
                return true;
            }
...
//                String str = HostRunStatusEnum.getName(info.getHostType());
                ...
                String format = String.format(StatusInfoChangeTypeEnum.event_CTC_AB.getDescr(), info.getAssetIp(), strOld);
