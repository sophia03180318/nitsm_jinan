...
public class SensorTemperatureFilterHandler extends IFilterHandler<CollectSensorEntity> {
    ...
    public boolean handler(CollectSensorEntity info) {
        ...
        if (flag1) {
            ...
            List<String> normalStatusList = Arrays.asList("0", "1","正常","OK");
            ...
            IEvent event1 = eventInfoChangeManagerService.creatChangeEvent(info.getAssetId(), changeInfo1, eventRedisKey1, eventMapKey1, status1, alarmTempReq,info.getInspectRecordId(),info.getVersion());
            if (event1 != null) {
                //被事件信息截取
