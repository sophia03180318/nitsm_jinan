...
public class CpuFilterHandler extends IFilterHandler<CollectCpuEntity> {
    ...
    public boolean handler(CollectCpuEntity info) {
        ...
        if (flag || thresholdFlag) {
            ...
            IEvent event = eventInfoChangeManagerService.creatChangeEvent(info.getAssetId(), changeInfo, eventRedisKey, eventMapKey, status, alarmTempReq, info.getInspectRecordId(),info
                    .getVersion());
            if (event != null) {
                //被事件信息截取
