 ...
 */
@Component("interfaceRxPowerStageFilterHandler")
public class InterfaceRxPowerStageFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        ...
        String mapkey = StatusInfoChangeTypeEnum.status_interface_rxPower.getCode();
        ChangeInfo changeInfo = info.getMaps().get(mapkey);
        if (changeInfo == null || changeInfo.getValue() == null) {
            return true;
        }

        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_optical_in_section.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_optical_in_section.getCode(), StatusInfoChangeTypeEnum.SECTION.getCode(), info.getPortName());

        ThresholdBaseEntity threshold = thresholdManager.getThresholdValue(StatusInfoChangeTypeEnum.event_port_optical_in_normal.getCode(), info.getAssetId(), info.getPortName()); // event:event_port:optical_in_normal
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_optical_in_section.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getMinValue(), threshold.getMaxValue());
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_optical_in_section.getCode(), StatusInfoChangeTypeEnum.SECTION_VAL.getCode(), info.getPortName(), status, info, changeInfo);
