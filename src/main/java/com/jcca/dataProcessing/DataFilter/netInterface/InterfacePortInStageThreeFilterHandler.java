 ...
 */
@Component("interfacePortInStageThreeFilterHandler")
public class InterfacePortInStageThreeFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        ...
        String mapkey = StatusInfoChangeTypeEnum.status_interface_portInRate.getCode();
        ChangeInfo changeInfo = info.getMaps().get(mapkey);
        if (changeInfo == null || changeInfo.getValue() == null) {
            return true;
        }

        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_in_sectionThree.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_in_sectionThree.getCode(),StatusInfoChangeTypeEnum.SECTION_THREE.getCode(),info.getPortName());

        ThresholdBaseEntity threshold = thresholdManager.getThresholdValue(StatusInfoChangeTypeEnum.event_port_in_normal.getCode(), info.getAssetId(), "");
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_in_sectionThree.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getThreeLevelValue());
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_in_sectionThree.getCode(),StatusInfoChangeTypeEnum.SECTION_THREE_VAL.getCode(),info.getPortName(), status, info, changeInfo);
