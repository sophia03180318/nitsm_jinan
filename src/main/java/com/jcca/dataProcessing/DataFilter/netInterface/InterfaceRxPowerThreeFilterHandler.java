 ...
 */
@Component("interfaceRxPowerThreeFilterHandler")
public class InterfaceRxPowerThreeFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        ...
        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_optical_in_sectionThree.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_optical_in_sectionThree.getCode(), StatusInfoChangeTypeEnum.SECTION_THREE.getCode(), info.getPortName());
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_optical_in_sectionThree.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getThreeLevelValue());

            AlarmTempReq tempReq = new AlarmTempReq();
            tempReq.setOrgMsg(descStr);
            tempReq.setThresholdValue(threshold.getThreeLevelValue()+"dbm");
            tempReq.setCollectValue(changeInfo.getValue()+"dbm");
            tempReq.setFlag(info.getPortName());

            //添加状态监控（设备监控的事件信息是否正常）
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_optical_in_sectionThree.getCode(), StatusInfoChangeTypeEnum.SECTION_THREE_VAL.getCode(), info.getPortName(), status, info, changeInfo);
