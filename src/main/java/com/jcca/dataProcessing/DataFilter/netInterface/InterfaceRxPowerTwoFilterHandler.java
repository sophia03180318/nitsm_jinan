 ...
 */
@Component("interfaceRxPowerTwoFilterHandler")
public class InterfaceRxPowerTwoFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        ...
        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_optical_in_sectionTwo.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_optical_in_sectionTwo.getCode(), StatusInfoChangeTypeEnum.SECTION_TWO.getCode(), info.getPortName());
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_optical_in_sectionTwo.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getTwoLevelValue());

            AlarmTempReq tempReq = new AlarmTempReq();
            tempReq.setOrgMsg(descStr);
            tempReq.setThresholdValue(threshold.getTwoLevelValue()+"dbm");
            tempReq.setCollectValue(changeInfo.getValue()+"dbm");
            tempReq.setFlag(info.getPortName());

            //添加状态监控（设备监控的事件信息是否正常）
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_optical_in_sectionTwo.getCode(), StatusInfoChangeTypeEnum.SECTION_TWO_VAL.getCode(), info.getPortName(), status, info, changeInfo);
