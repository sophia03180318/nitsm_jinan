...
/**
 * @author Zhaozheng
 * @description TODO 端口流入普通阈值信息过滤处理类
 * @className InterfaceFilterHandler
 * @date 2023/10/27 9:36
 * @since 2.1.0.0
 */
@Component("interfacePortInFilterHandler")
public class InterfacePortInFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        ...
        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_in_normal.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_in_normal.getCode(),StatusInfoChangeTypeEnum.NORMAL.getCode(),info.getPortName());
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_in_normal.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getBaseValue());
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_in_normal.getCode(),StatusInfoChangeTypeEnum.NORMAL_VAL.getCode(),info.getPortName(), status, info, changeInfo);
