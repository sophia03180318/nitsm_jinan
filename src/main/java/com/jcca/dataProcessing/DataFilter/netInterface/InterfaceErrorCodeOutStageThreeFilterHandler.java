 ...
 */
@Component("interfaceErrorCodeOutStageThreeFilterHandler")
public class InterfaceErrorCodeOutStageThreeFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        ...
        String mapkey = StatusInfoChangeTypeEnum.status_interface_errorCodeOutRate.getCode();
        ChangeInfo changeInfo = info.getMaps().get(mapkey);
        if (changeInfo == null || changeInfo.getValue() == null) {
            return true;
        }

        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_outError_sectionThree.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_outError_sectionThree.getCode(),StatusInfoChangeTypeEnum.SECTION_THREE.getCode(),info.getPortName());

        ThresholdBaseEntity threshold = thresholdManager.getThresholdValue(StatusInfoChangeTypeEnum.event_port_outError_normal.getCode(), info.getAssetId(),"");
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_outError_sectionThree.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getThreeLevelValue());
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_outError_sectionThree.getCode(),StatusInfoChangeTypeEnum.SECTION_THREE_VAL.getCode(),info.getPortName(), status, info, changeInfo);
