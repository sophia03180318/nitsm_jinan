...
/**
 * @description: 端口流入区间阈值 大于最大或小于最小
 * @author: Lvyp
 * @create: 2023/11/02 19:59
 */
@Component("interfacePortInSectionFilterHandler")
public class InterfacePortInSectionFilterHandler extends IFilterHandler<CollectInterfaceEntity> {
    ...
    public boolean handler(CollectInterfaceEntity info) {
        String mapkey = StatusInfoChangeTypeEnum.status_interface_portInSpeedBps.getCode();
        ChangeInfo changeInfo = info.getMaps().get(mapkey);
        if (changeInfo == null || changeInfo.getValue() == null) {
            return true;
        }

        String eventRedisKey = StatusInfoChangeTypeEnum.event_port_in_section.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_port_in_section.getCode(),StatusInfoChangeTypeEnum.SECTION.getCode(),info.getPortName());

        ThresholdBaseEntity threshold = thresholdManager.getThresholdValue(StatusInfoChangeTypeEnum.event_port_in_normal.getCode(), info.getAssetId(), info.getPortName());
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            String descStr = String.format(StatusInfoChangeTypeEnum.event_port_in_section.getDescr(), info.getPortName(), changeInfo.getValue(), keyWord, threshold.getMinValue(), threshold.getMaxValue());
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_port_in_section.getCode(),StatusInfoChangeTypeEnum.SECTION_VAL.getCode(),info.getPortName(), status, info, changeInfo);
