 ...
 */
@Component("diskStageThreeFilterHandler")
public class DiskStageThreeFilterHandler extends IFilterHandler<CollectDiskEntity> {
    ...
    public boolean handler(CollectDiskEntity info) {
        AppLogUtils.buildLogInfo(LogFunctionEnum.DATA_PROCESS_SINGLE, "磁盘三阶阈值过滤处理类", info.getAssetIp());
        ChangeInfo changeInfo = info.getMaps().get(info.getName());

        String eventRedisKey = StatusInfoChangeTypeEnum.event_disk_sectionThree.getCode();
        ...
        String thresholdMapKey = thresholdManager.getThresholdMapKey(StatusInfoChangeTypeEnum.event_disk_sectionThree.getCode(),StatusInfoChangeTypeEnum.SECTION_THREE.getCode(),info.getName());
        ...
        if (changeInfo.getIsChange() || thresholdFlag) {
            ...
            alarmTempReq.setOrgMsg(String.format(StatusInfoChangeTypeEnum.event_disk_sectionThree.getDescr(), info.getName(), changeInfo.getValue(), keyWord, threshold.getThreeLevelValue()));
            ...
            this.addEventStatus(StatusInfoChangeTypeEnum.event_disk_sectionThree.getCode(),StatusInfoChangeTypeEnum.SECTION_THREE_VAL.getCode(),info.getName(), status, info, changeInfo);
            if (event != null) {
                //被事件信息截取
                changeInfo.setIsEvent(true);
                event.setDescStr(String.format(StatusInfoChangeTypeEnum.event_disk_sectionThree.getDescr(), info.getName(), changeInfo.getValue(), keyWord, threshold.getThreeLevelValue()));
                this.dispatureEvent(event);
            }
