 ...
 */
@Slf4j
public class ServersRecordExcelUtil {
    ...
    public static SXSSFWorkbook createExcel(AssetRecordReq req) {
        ...
        cell.setCellValue("CTC/TDCS" + req.getPlace().getMsg() + "服务器履历动态记录表");
        ...
        row5.createCell(0).setCellValue("服务器运行模式：" + req.getRunModel());
        ...
        row6.createCell(1).setCellValue("服务器型号：" + req.getAssetImage());
        ...
        row9.createCell(1).setCellValue("电源模块数量：" + req.getPowerTotal());
        row9.createCell(4).setCellValue("电源模块型号：" + req.getPowerModel());
    ...
    public static void responseBody(SXSSFWorkbook workbook, HttpServletResponse response) {
        // response
        try {
            String fileName = URLEncoder.encode("服务器履历动态记录表.xlsx", "UTF-8");
