...
public interface TopoVertexMapper extends BaseMapper<TopoVertex> {
     ...
     * @return
     */
    @Select("SELECT V.*,t.show_core as showCore, T.NAME AS ASSETNAME, T.ID AS ASSETID, T.ORG_ID AS ASSETORGID, ASSET_MODE AS ASSETMODE FROM ASSET T LEFT JOIN TOPO_VERTEX V ON  T.ID = V.ASSET_ID AND #{reqID}=V.ORG_ID WHERE T.ORG_ID IN (SELECT ID FROM SYS_ORG WHERE PID =#{reqID}) AND T.ASSET_MODE =42 AND T.SHOW_TOPO=1 AND T.IS_DEL=1")
     ...
     * @return
     */
    @Select("SELECT V.*,t.show_core as showCore, T.NAME AS ASSETNAME, T.ID AS ASSETID, T.ORG_ID AS ASSETORGID, ASSET_MODE AS ASSETMODE FROM ASSET T LEFT JOIN TOPO_VERTEX V ON  T.ID = V.ASSET_ID AND #{reqID}=V.ORG_ID WHERE T.ORG_ID IN (SELECT ID FROM SYS_ORG WHERE PID =#{reqID}) AND T.ASSET_MODE =201 AND T.SHOW_TOPO=1 AND T.IS_DEL=1")
