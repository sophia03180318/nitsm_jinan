...
@Service
@Slf4j
public class SwitchHuaweiConfigAdapterImpl implements XunjianV2Adapter {
    /**
     * 含有|PWD|标识的表示需要进入特权模式，需要输入密码
     */
    private static final String command = "sysTPWDTdis cu";
    private static final String commandName = "备份华为交换机配置";
    private static final List<Integer> HUAWEI_MANUFACTURER_IDS = Arrays.asList(2,6,8);

    @Override
    public String getCode() {
        return XunjianItemCode.HUAWEI_CONFIG;
    }

    @Override
    public String getName(Asset asset) {
        if(!HUAWEI_MANUFACTURER_IDS.contains(asset.getManufacturerId())){
            return "";
        }
        return commandName;
    }
