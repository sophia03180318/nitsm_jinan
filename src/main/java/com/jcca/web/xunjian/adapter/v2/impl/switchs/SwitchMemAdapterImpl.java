...
@Service
@Slf4j
public class SwitchMemAdapterImpl implements XunjianV2Adapter {

    private static final String command = "mib采集";
    private static final String commandName = "内存使用率";
    ...
    public String getCode() {
        return XunjianItemCode.SWITCH_MEMORY;
    }
    ...
    public XunjianDetailV2 xunJian(Asset asset, String xunjianRecordId ,String orgMsg) {
        ...
        try {
            List<CollectMemory> realTimeData = collectMemoryService.getRealTimeData(asset.getId());
            StringBuilder orgStr = new StringBuilder("");
            for (CollectMemory item : realTimeData) {
                orgStr.append("内存使用率：");
                orgStr.append(item.getMemUsedRate());
                if(item.getMemUsedRate()>max){
                    xunjianDetail.setInputOrgStr("内存使用率"+item.getMemUsedRate()+"大于"+max);
