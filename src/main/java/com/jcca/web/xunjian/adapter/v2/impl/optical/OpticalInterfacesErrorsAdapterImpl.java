...
@Service
@Slf4j
public class OpticalInterfacesErrorsAdapterImpl implements XunjianV2Adapter {

    private static final String command = "porterrshow";
    private static final String commandName = "查看交换机端口的错误统计";
    ...
    public String getCode() {
        return XunjianItemCode.OPTICAL_INTERFACES_ERRORS;
    }
    ...
    public XunjianDetailV2 xunJian(Asset asset, String xunjianRecordId,String orgMsg) {
        ...
        try {
            ...
            for (String line : lineArray) {
                ...
                if(lineLower.contains(":")&&!lineLower.contains(">")){
                    String[] cellArray = lineLower.split("\\s+");
                    String linkFail = cellArray[11];
                    String lossSync = cellArray[12];
                    String lossSig = cellArray[13];

                    if(Integer.valueOf(linkFail)>max||Integer.valueOf(lossSync)>max||Integer.valueOf(lossSig)>max){
                        errorStr.append("【link fail:");
                        errorStr.append(linkFail);
                        errorStr.append(",loss sync");
                        errorStr.append(lossSync);
                        errorStr.append(",loss sig");
                        errorStr.append(lossSig);
                        errorStr.append("】");
                    }
                }
            }
