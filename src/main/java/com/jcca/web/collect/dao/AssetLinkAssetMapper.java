...
public interface AssetLinkAssetMapper extends BaseMapper<AssetLinkAsset> {
    ...
    @Select("SELECT * FROM (SELECT a.NAME assetName, a.ip assetIp, l.port_index, l.link_asset_name, l.link_asset_ip, l.link_port, l.remark FROM ASSET_LINK_ASSET l LEFT JOIN ASSET a ON l.ASSET_ID = a.ID order by a.name, l.port_index) t WHERE t.assetName is not null")
