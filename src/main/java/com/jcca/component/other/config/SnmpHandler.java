...
/**
 * snmp被动告警监视，需配置管理口指到采集器上
 * 默认监听162端口
 */

/**
 * @ClassName SnmpHandler
 * @Description snmp告警处理器
 * @Date 2020/7/9 10:39
 * @Author hanwone
 */
@Component
@Slf4j
public class SnmpHandler implements CommandResponder {

    @Resource
    private SnmpEventService snmpAlarmService;
...
//        log.info("【SNMP收到原始信息】：{}", JSONUtil.toJsonStr(event));
        ...
        if (!snmpMap.values().isEmpty()) {

            ReceiveAlarmDto snmpDto = new ReceiveAlarmDto();
            snmpDto.setAssetIp(event.getPeerAddress().toString().split("/")[0]);
            snmpDto.setCategory(ReceiveAlarmTypeEnum.SNMP.getCode());
            snmpDto.setOccurTime(DateUtil.formatDateTime(new Date()));
            snmpDto.setContent(snmpMap.toString());

            snmpAlarmService.addEvent(snmpDto);
        }
    }
