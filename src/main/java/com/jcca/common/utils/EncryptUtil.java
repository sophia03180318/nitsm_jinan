...
public class EncryptUtil {
     ...
     */
    public static String encrypt(String password, String salt, String hashAlgorithmName, int hashIterations) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        // 将字符串转换为字节数组
     ...
     * @return 16进制密文
     */
    public static String aesEncryptHex(String pwd) {
        if (StrUtil.isEmpty(pwd)) {
            return "";
        }

        String key = "wKQnI14qpqbRLVXG4VJvyNycIPviu2Hp";
        return SecureUtil.aes(key.getBytes()).encryptHex(pwd.getBytes());
    }
     ...
     * @return 字符串明文
     */
    public static String aesDecryptStr(String cipher) {
        if (StrUtil.isEmpty(cipher)) {
            return "";
        }
        String key = "wKQnI14qpqbRLVXG4VJvyNycIPviu2Hp";
        return SecureUtil.aes(key.getBytes()).decryptStr(cipher);
    }
}
