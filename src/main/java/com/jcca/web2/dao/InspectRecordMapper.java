...
//    List<InspectOrgAssetVo> findOrgList(@Param("orgIds") List<String> orgIds);
    ...
    @Select("SELECT ASSET_ID, ASSET_NAME, ORG_ID, ASSET_DESK, ASSET_STATUS FROM INSPECT_RECORD WHERE ORG_ID = #{orgId} AND ASSET_DESK = #{assetDesk} GROUP BY ASSET_ID, ASSET_NAME, ORG_ID, ASSET_DESK, ASSET_STATUS ORDER BY ASSET_NAME")
