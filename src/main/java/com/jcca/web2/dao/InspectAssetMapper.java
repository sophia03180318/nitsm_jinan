...
public interface InspectAssetMapper extends BaseMapper<InspectAsset> {
    ...
    @Select("SELECT t.EVENT_CATEGORY AS ID, t.TYPE_ALIAS AS NAME, MAX(INSPECT_STATE) AS STATUS, COUNT(*) AS TOTAL, " +
            "SUM(CASE WHEN i.INSPECT_STATE = 3 THEN 1 ELSE 0 END) AS NORMAL, " +
            "SUM(CASE WHEN i.INSPECT_STATE = 4 THEN 1 ELSE 0 END) AS ABNORMAL " +
            "FROM INSPECT_ASSET i LEFT JOIN ALARM_EVENT_TYPE t on i.EVENT_TYPE_ID = t.ID " +
