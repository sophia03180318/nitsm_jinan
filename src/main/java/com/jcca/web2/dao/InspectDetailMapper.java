...
public interface InspectDetailMapper extends BaseMapper<InspectDetail> {
    ...
    @Select("SELECT INSPECT_CODE, INSPECT_TIME createTime, RESULT_PATH FROM INSPECT_DETAIL WHERE INSPECT_STATE != 1 GROUP BY INSPECT_CODE, INSPECT_TIME, RESULT_PATH ORDER BY INSPECT_TIME DESC")
    List<InspectRecordListVo> findRecordList();

    @Select("SELECT INSPECT_CODE, ASSET_ID, ASSET_NAME FROM INSPECT_DETAIL WHERE INSPECT_STATE != 0 AND INSPECT_CODE = #{inspectCode} GROUP BY INSPECT_CODE, ASSET_ID, ASSET_NAME ORDER BY ASSET_ID")
