...
function mxGraph(container, model, renderHint, stylesheet) {
    ...
    // 在原型被修改为保存一些监听器的情况下初始化变量
    // 这是可能的，因为无论传递给ctor的参数如何都要执行createHandlers调用。
    this.mouseListeners = null;

    // Converts the renderHint into a dialect
    // 将renderHint转换为dialect
    // 设置渲染cell的语言
 ...
 */
// mxGraph会在init方法调用之前创建几个句柄用于处理对应的事件，
// 有如下几个句柄：tooltip、panning、connection、graph，不过目前都是关闭的：
 ...
 */
// 当cell重叠时，mxGraph需要额外进行一些操作记录重叠的cell并更新graph的显示，
//这样才能实现cell重叠层次的改变：
/**
 * cell - 被叠加的mxCell
...
mxGraph.prototype.createVertexAsset = function (parent, id, value,
                                                x, y, width, height, style, relative, assetId, assetType, connectable, offset, isPort,portIndex,portMode) {
    // Creates the geometry for the vertex
    var geometry = new mxGeometry(x, y, width, height);
    geometry.relative = (relative != null) ? relative : false;
    if (offset != null) {
    ...
    vertex.setAssetId(assetId);
    vertex.setAssetType(assetType);
    vertex.setIsPort(isPort);
    vertex.setPortIndex(portIndex);
    vertex.setPortMode(portMode)
   return this.addCell(vertex, parent);
};
...
//添加自定义连线
mxGraph.prototype.createEdgeAsset = function (parent, id, value, source, target, style, points, assetId, assetType, position) {
    // Creates the edge
    var edge = new mxCell(value, new mxGeometry(), style);
    edge.setId(id);
    edge.setAssetType(assetType);
    edge.setAssetId(assetId);
    edge.setEdge(true);
